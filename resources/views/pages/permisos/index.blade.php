            ...
            <tbody id="permisos-crud">
              ...
              <tr id="permisos_id_{{$permiso->id}}">
                ...
                <td>
                  ...
                  <a id="borrar-permiso" data-id="{{$permiso->id}}" type="button" class="btn btn-danger">Eliminar</a>
...
@push('custom-scripts')
  ...
  <script>
    $(function() {
      ...
      $(function() {
        $("#permisoForm").validate({
          ...
          messages: {
            descripcion: {
              required: "Por favor, introduzca un permiso"
            }
          },
    ...
    $(document).ready(function () {
      ...
      $('body').on('click', '#borrar-permiso', function () {
        var id = $(this).data("id");
        swal.fire({
          title: "¿Eliminar?",
      ...
      $('body').on('click', '#btn-save', function () {
        ...
        $.ajax({
          ...
          dataType: 'json',
          success:function (data) {
            var post = '<tr id="permiso_id_' + data[0].id + '"><td>' + data[0].id + '</td><td>' + data[0].descripcion + '</td>';
            post += '<td><a href="javascript:void(0)" id="editar-permiso" data-id="' + data[0].id + '" class="btn btn-outline-dark">Editar</a>';
            post += '<a href="javascript:void(0)" id="borrar-permiso" data-id="' + data[0].id + '" class="btn btn-danger delete-post">Eliminar</a></td></tr>';
            // alert("Llegaste");
            if (actionType == "crearPermiso") {
              $('#permisos-crud').prepend(post);
            } else {
              $("#permisos_id_" + data[0].id).replaceWith(post);
            }
            $('#permisoForm').trigger("reset");
